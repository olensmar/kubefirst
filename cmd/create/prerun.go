...
func validateCreate(cmd *cobra.Command, args []string) error {
	...
	// if non-development/built/released version, set template tag version to clone tagged templates
	// the built version uses the same template repository version.
	// example: kubefirst version 1.10.3, has template repositories (gitops and metaphor's) tags set as 1.10.3
	// when Kubefirst downloads the templates, it will download the tag version that matches Kubefirst version
	...
	viper.Set("gitops-template.repo", gitOpsRepo)
	viper.Set("gitops-template.owner", "kubefirst")  //*
